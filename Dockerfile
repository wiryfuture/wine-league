...
# Build packages and export to folder
CMD cd /builddir/out64 && /builddir/temp/configure --enable-win64 && ccache make -j$(expr $(nproc) \+ 1) &&\
cd /builddir/out32 && PKG_CONFIG_PATH=/usr/lib/pkgconfig /builddir/temp/configure --with-wine64=/builddir/out64 && ccache make -j$(expr $(nproc) \+ 1) &&\
mv -r /builddir/out32/. /exports/.
