...
RUN cd /builddir/out32 && PKG_CONFIG_PATH=/usr/lib/pkgconfig ../wine/configure && ccache make -j$(expr $(nproc) \+ 1)
...
RUN mkdir /builddir/build && mv /builddir/out32 /builddir/build && mv /builddir/wine /builddir/build 
