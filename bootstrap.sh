...
# By default, we're building 64bit and 32bit, so we need 64bit deps
...
if [ "$BUILD64BIT" != 0 ] ; then
    mkdir -p /builddir/wine /builddir/out64 /builddir/out32 /builddir/patches /builddir/wine-staging 
    else 
    mkdir -p /builddir/wine /builddir/out32 /builddir/patches /builddir/wine-staging 
fi
